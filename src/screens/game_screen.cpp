...
ScreenType GameScreen::display()
{
    ...
    node->setMaterialTexture(0, this->driver->getTexture((CONFIG.BASE_FOLDER + "gfx/map_texture.png").c_str()));
    // node->setRotation(node->getPosition().rotationToDirection(irr::core::vector3df(INPUT.mouseInput.X, INPUT.mouseInput.Y * -1.0f, 0)));
    node->setRotation(node->getPosition().rotationToDirection(irr::core::vector3df(-30, 0, 0)));
